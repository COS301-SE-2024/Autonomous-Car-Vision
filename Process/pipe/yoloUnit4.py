...
class SegUnit(Unit):
    ...
    def to_tensorrt(self):
        ...
        profile.set_shape(network.get_input(0).name,
                          (1, 3, 640, 640),  # Minimum shape
                          (1, 3, 640, 640),  # Optimal batch size
                          (1, 3, 640, 640))  # Maximum shape
    ...
    def trt_process(self, frame):
        ...
        # Get the tensor names and set the tensor addresses
        input_binding_index = self.trt_engine.get_binding_index('input')
        output_binding_index = self.trt_engine.get_binding_index('output')

        # Set the input shape
        self.context.set_binding_shape(input_binding_index, frame.shape)

        # Set tensor addresses
        self.context.set_tensor_address(input_binding_index, int(self.d_input))
        self.context.set_tensor_address(output_binding_index, int(self.d_output))

        # Perform inference
        cuda.memcpy_htod_async(self.d_input, frame, stream)
        self.context.execute_async_v3(stream.handle)

        output_data = np.empty((1, 3, 640, 640), dtype=np.float32)
        cuda.memcpy_dtoh_async(output_data, self.d_output, stream)
        stream.synchronize()
