<script>
    ...
    async function encoder() {
        ...
        // Split the pipe string by commas and remove 'N-' prefix from each unit
        ...
        // Extract the intermediate units (nodes)
        const intermediateUnits = units.slice(1, -1);

        // Retrieve labels for each intermediate unit
        ...
        // Construct the final pipe string with labels
...
<ProtectedRoutes>
    ...
    <div class="toolbar flex flex-row justify-between">
        <div class="flex flex-row items-center gap-4">
            ...
            <select class="bg-dark-primary rounded-full text-dark-background text-center"
        ...
        <div class="flex flex-row gap-2">
            <!-- <Button on:click={LoadCanvas} class="bg-dark-primary text-dark-background"
              >Load Prev</Button
            > -->
            <Button rounded on:click={SaveCanvas} class="bg-dark-primary text-dark-background"
            >Save Pipe
            </Button
            >
            <Button rounded
                    on:click={ClearCanvas}
                    class="bg-dark-primary text-dark-background">Clear Pipe
            </Button
            >
        </div>
    </div>
