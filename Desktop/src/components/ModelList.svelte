...
<div class="absolute top-0 right-0 bg-theme-dark-backgroundBlue border border-gray-300 shadow-lg p-4 z-50 flex flex-col rounded-lg">
    {#each models as model}
        <!-- svelte-ignore a11y-click-events-have-key-events -->
        <div class="mb-2 cursor-pointer flex items-center rounded border-b border-theme-dark-primary text-theme-dark-primary" on:click={() => selectModel(model)}>
            <img src={model.profileImg} alt="Model Profile" class="w-12 h-12 rounded-full mr-2">
            <p class="text-xs text-theme-dark-primary">{model.label}</p>
        </div>
    {/each}
</div>