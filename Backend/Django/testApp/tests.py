...
class ModelsTestCase(TestCase):
    ...
    def setUp(self):
        self.user1 = User.objects.create(uid=1, uname="testuser1", uemail="testuser1@example.com")
        self.user2 = User.objects.create(uid=2, uname="testuser2", uemail="testuser2@example.com")
        ...
        self.otp1 = OTP.objects.create(uid=self.user1, otp="123456", creation_date=timezone.now(), expiry_date=timezone.now() + datetime.timedelta(minutes=5))
        self.otp2 = OTP.objects.create(uid=self.user2, otp="654321", creation_date=timezone.now(), expiry_date=timezone.now() + datetime.timedelta(minutes=5))
        
        self.token1 = Token.objects.create(uid=self.user1, token="token1")
        self.token2 = Token.objects.create(uid=self.user2, token="token2")
        
        self.media1 = Media.objects.create(uid=self.user1, mid="media1", media_name="Media 1", media_url="media/media1.mp4")
        self.media2 = Media.objects.create(uid=self.user2, mid="media2", media_name="Media 2", media_url="media/media2.mp4")
    ...
    def test_user_unique_constraints(self):
        with self.assertRaises(Exception):
            User.objects.create(uid=1, uname="duplicateuser", uemail="duplicate@example.com")
    ...
    def test_media_unique_constraints(self):
        with self.assertRaises(Exception):
            Media.objects.create(uid=self.user1, mid="media1", media_name="Duplicate Media", media_url="media/media1.mp4")
