...
class TestFastAPIApp(unittest.TestCase):
    ...
    def test_getHardwareInfo_nvcc_unexpected_result(self, mock_run, mock_which):
        ...
        mock_run.side_effect = [
            subprocess.CompletedProcess(args=['nvidia-smi'], returncode=0, stdout="NVIDIA-SMI"),
            subprocess.CompletedProcess(args=['nvcc', '--version'], returncode=0, stdout="Unexpected result")
        ]
        self.assertFalse(getHardwareInfo())
    ...
    def test_process_endpoint_success(self, mock_getHardwareInfo):
        ...
        response = self.client.post("/process/", json={"uid": "123", "mid": "456", "token": "789"})
        self.assertEqual(response.status_code, 200)
